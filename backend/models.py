...
class PostModel:
    """Visual, Canva/Instagram-style posts. Each post stores the full Fabric.js
    canvas as JSON (so it stays editable later) plus a rendered PNG thumbnail
    (a data URL) so grids/feeds can display it without re-rendering the canvas."""

    @classmethod
    def list_public(cls, limit=60):
        return db.fetchall(
            """SELECT posts.id, posts.user_id, posts.title, posts.thumbnail,
                      posts.created_at, posts.updated_at,
                      profiles.display_name, users.full_name
               FROM posts
    ...
    def list_for_user(cls, user_id):
        return db.fetchall(
            """SELECT id, user_id, title, thumbnail, created_at, updated_at
    ...
    def update(cls, post_id, title, canvas_json, thumbnail, width, height):
        return db.execute(
            """UPDATE posts SET title = ?, canvas_json = ?, thumbnail = ?,
